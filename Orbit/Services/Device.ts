...
post("/Device",(req, res, next) => {
    //create/update new device ~ ..
    throwIf(!(
        validator.isLength(req.param("did"), 5, 35) &&
        validator.isLength(req.param("busname"), 1, 35) &&
        validator.isLength(req.param("hwaddr"), 1, 35)
        ));

    var dev: Storage.Models.Device.IDevice;
    if (!req.device) {
