...
var spec;
  var Kick = function ( o ) {
    ...
    this.threshold = o.threshold !== undefined ? o.threshold :  0.15;
    this.decay     = o.decay     !== undefined ? o.decay     :  0.01;
  ...
  Kick.prototype = {
    ...
    onUpdate : function () {
      ...
      if ( magnitude >= this.currentThreshold &&
        ...
        this.onKick && this.onKick.call( null, magnitude );
      } else {
        this.offKick && this.offKick.call( null, magnitude );
...
var f = new FFT(1024, 44100);


var cp = require("child_process").spawn;

var audio = cp("pacat", ["-r", "--raw", "--format=float32le", "--latency-msec=1", "-d", "bluez_source.28_47_AA_26_59_6D", "--channels=1", "--rate=44100"]);

var current = [];

function avg(min, max, amp) {
	var t = 0;
...
audio.stdout.on("data", function(d){
	...
	if(current.length == 1024){
		...
		if(state > 0){
			state--;
			if(avg(300,500,14500) > 100){
				go(0, 60, 190, _m * 800);
			} else {
				go(255, 9, 0, _m * 800);
			}
		} else {
			go(avg(200,500,135600),avg(300,500,106500),avg(60,120,14500),avg(0,512,3000) + 30);
		}
	} else {
		current.push(d.readFloatLE(i));
	}
    }
});
