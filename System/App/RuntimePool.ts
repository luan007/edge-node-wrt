...
function ConnectionHandler(
    ...
    for (var i = 0; i < keys.length; i++) {
        ...
        if (runtime.Status().State === 1 &&
            ...
            Core.API.Permission.SetPermission(
                runtime.RuntimeId,
                Core.API.Permission.Encode([
                    Permission.AppPreLaunch
                ])
            );
            //Start serving STUFF
            return Core.API.Server.Serve(socket, CONF.SENDER_TYPE_APP, runtime.RuntimeId, runtime.RuntimeId, (err, endpoint) => {
...
export function Initialize() {
    ...
    SYS_ON(SYS_EVENT_TYPE.LOADED, function () {
        ...
        Core.SubSys.Native.user.ClearGenerated((err, result) => {
            if (err) {
            ...
            Core.Data.Tables.Application().all({}, (err, results) => {
                if (err) {
                    return error(err);
                } else {
