...
function S.fcntl(fd, cmd, arg)
    ...
    if cmd == S.F_GETFD or cmd == S.F_GETFL or cmd == S.F_GETLEASE or cmd == S.F_GETOWN or
            cmd == S.F_GETSIG or cmd == S.F_GETPIPE_SZ then return retnum(ret)
    end
    return retbool(ret)
end
...
function S.uname()
    ...
    if ret == -1 then return errorret() end
    return {
        sysname = ffi.string(u.sysname),
        nodename = ffi.string(u.nodename),
        release = ffi.string(u.release),
        version = ffi.string(u.version),
        machine = ffi.string(u.machine),
        domainname = ffi.string(u.domainname)
    }
end
...
if ret == -1 then return errorret() end
return {
    readfds = fdisset(s.readfds or {}, r),
    writefds = fdisset(s.writefds or {}, w),
    exceptfds = fdisset(s.exceptfds or {}, e),
    count = tonumber(ret)
}
end
...
elseif type(rlim) == 'table' then rlim = rlimit_t(rlim)
end
...
-- aio functions
local function getctx(ctx) return ulong_t(ctx.ctx) end

-- aio_context_t is really unsigned long
...
-- methods on an fd
local fdmethods = {
    'nogc', 'nonblock', 'block', 'sendfds', 'sendcred',
...
S.t = {
    ...
    timespec = timespec_t,
    buffer = buffer_t,
    stat = stat_t, -- not clear if type for fd useful
    sockaddr = sockaddr_t,
    sockaddr_in = sockaddr_in_t,
    in_addr = in_addr_t,
    utsname = utsname_t,
    sockaddr_un = sockaddr_un_t,
    iovec = iovec_t,
    msghdr = msghdr_t,
    cmsghdr = cmsghdr_t,
    timeval = timeval_t,
    ...
    uint64 = uint64_t,
    macaddr = macaddr_t,
    sockaddr_storage = sockaddr_storage_t,
    sockaddr_in6 = sockaddr_in6_t,
    pollfds = pollfds_t,
    epoll_events = epoll_events_t,
