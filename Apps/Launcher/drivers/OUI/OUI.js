...
exports.update_scheduler = update_scheduler;
function Rebuild(fileName, cb) {
    console.log("Rebuild Scheduled - Expect delays");
    hotswap("OUI_DB", function (done) {
...
function OUI_Find(mac, callback) {
    hotswapSafe("OUI_DB", callback, function (done) {
        db.get(mac.toLowerCase(), {}, done);
    });
}
exports.OUI_Find = OUI_Find;
function Initialize(cb) {
    ...
    db = level(dbPath, { 'createIfMissing': true, 'valueEncoding': 'json' }, function () { });
    db.open();
    if (!rebulid) {
        return cb();
    }
